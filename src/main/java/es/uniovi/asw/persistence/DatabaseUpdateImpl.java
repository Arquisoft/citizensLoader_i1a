...
public class DatabaseUpdateImpl implements DatabaseUpdate {
	...
	@Override
	public void addToDatabase(User u, String filename) throws IOException {
		EntityManager em = Jpa.getEntityManager();
		EntityTransaction trx = em.getTransaction();
		trx.begin();
		if (doesUserExist(u)) {
			logger.log(filename, "User " + u.getFirstName() + " " + u.getLastName() + " already exists");
			if (!doesUserMatch(u)) {
				logger.log(filename, "The user " + u.getFirstName() + " " + u.getLastName() + " data in the "
	...
	public boolean doesUserExist(User user) {
		String query = "SELECT COUNT(u)" + " FROM User u" + " WHERE u.identification = ?1";
	...
	public boolean doesUserMatch(User user) {
		...
		Long result = (Long) Jpa.getEntityManager().createQuery(query).setParameter(1, user.getIdentification())
				...
				.setParameter(6, user.getAddress()).setParameter(7, user.getNationality()).getSingleResult();
		return result == 0;
	}
